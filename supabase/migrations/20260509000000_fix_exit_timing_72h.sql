-- ============================================================
-- 퇴장 타이밍 수정: 48h → 경고, 총 72h 미인증 → 퇴장
--
-- 기존: ACTIVE → EXIT_ELIGIBLE (48h), exit_deadline = NOW() + 2h → 총 50h
-- 수정: ACTIVE → EXIT_ELIGIBLE (48h), exit_deadline = last_verified_at + 72h → 총 72h
-- ============================================================
...
BEGIN
  ...
  -- 마지막 인증(또는 가입)으로부터 48시간 경과 시 경고
  -- exit_deadline = last_verified_at + 72h (총 72시간 기준)
  WITH changed AS (
    UPDATE group_members
    SET member_status = 'EXIT_ELIGIBLE',
        exit_deadline = COALESCE(last_verified_at, joined_at) + INTERVAL '72 hours'
  ...
  SELECT
    ...
    'member_warning',
    '퇴장경고',
    '48시간 이상 미인증으로 퇴장경고 상태가 됐어요. 72시간이 될 때까지 인증하지 않으면 ' || g.name || '에서 퇴장돼요.',
    '⚠️',
    c.group_id::TEXT
  FROM changed c
  ...
  -- exit_deadline(= 가입/마지막인증 + 72h)이 지나면 퇴장
  WITH changed AS (
    UPDATE group_members
